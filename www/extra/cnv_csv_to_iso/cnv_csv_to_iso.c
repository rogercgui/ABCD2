 ...
 *
 *      Compile this program for linux the cc compiler:
 *              cc cnv_csv_to_iso.c -o cnv_csv_to_iso
 ...
 *      2024-12-11:(fho) Merged with version FH: Added replacing linefeeds by blanks in q00ReplaceChars
 *      2024-12-20:(fho) Replace linefeeds in non-repeating terms only. Remove leading spaces in arguments. Update comment
 *      2026-17-03:(fho) Add check for UTF-8 characters
 ...
 ******************************************************************************/ 
{
    ...
    ** Some installations add a leading space to optarg. This is removed
    */ 
    while ( ( option = getopt( argc, argv, "vhH8Si:d:o:c:r:" ) ) != -1 ) {
        switch ( ( char ) option ) {
            case 'i':
		while(isspace((unsigned char)*optarg)) optarg++;
                infile = optarg;
                break;
            case 'd':
		while(isspace((unsigned char)*optarg)) optarg++;
...
*/ 
{
        ...
        if ( (int)strlen(token) > 0 ) {
            ...
            */
            if ( reptoks[i] == 0 ) {
                /* 
                ** Non-repeating token:
		** Embedded linefeeds are replaced by a space.
		** The CSV allows embedded linefeeds, but the ISO importer not
                */
		int tokenlength = strlen(token);
                for (int i = 0; i < tokenlength; i++) {
                    if (token[i] == delimiterLF) {
                        token[i] = ' ';
                    }
                }
    ...
    */
    for ( i=0; i < numacttokens; i++ ) {
        /* debug
            fprintf(stderr, "data %d (%.3s)(%s)  %i\n",i, dir[i], dir[i],strlen(data[i]));
            fprintf(stderr, "data %d (%.3s)(%s)= %s\n",i, dir[i], dir[i],data[i]);
        */
...
*/ 
{
    ...
    int numgt = 0;
    int numutf = 0;
    unsigned char testc, testc2, testc3, testc4;
    ...
    for ( i=0; i < length; i++ ) {
        testc  = record[i];
	testc2 = 0;
	if ( i < length+1 ) testc2=record[i+1];
        if ( testc == '<' ) {
            numlt++;
            if ( CMDOPTS > 0 ) {
                record[i] = '«';
        ...
        } else if ( (testc&0xc0) == 0xc0 && (testc2&0xc0) == 0x80 ) {
	    /*
	    ** first test 11...... on testc, then test 10...... on testc2
	    ** This test is insufficient and may give false positives but very rare.
	    */
	    numutf++;
	    //fprintf(stderr,"value of chars: %x %x\n",testc, testc2);
	}
    ...
    if ( numutf > 0 ) {
        fprintf( stderr, "*** - Record %d: Contains %d possible UTF-8 characters.\n", recordnumber,numutf);
    }
