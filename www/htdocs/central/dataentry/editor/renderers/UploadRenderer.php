...
class UploadRenderer
{
    ...
    public static function renderInput($Etq, $campo, $rows, $cols, $style_input = "", $maxlength = 0)
    {
        ...
        if ($rows > 1) {
            ...
            echo " placeholder='" . ($msgstr["drag_drop"] ?? "Drag the files here") . "'>" . htmlspecialchars($campo ?? '', ENT_QUOTES, 'UTF-8') . "</textarea>";
        } else {
            echo "<input tabindex='0' type='text' name='tag" . $Etq . "' id='tag" . $Etq . "' size='" . $cols . "' class='abcd-upload-input' style=\"$style_input\" placeholder='" . ($msgstr["drag_drop"] ?? "Drag the files here") . "' value=\"" . htmlspecialchars($campo ?? '', ENT_QUOTES, 'UTF-8') . "\">";
        }
        echo "</div>";

        echo "<div style='display:flex; justify-content:flex-end; gap:6px; padding-top:4px; border-top: 1px solid #eee;'>";
        echo "<button type='button' class='abcd-btn-upload' onclick=\"ABCD_openUploadModal('tag$Etq')\" title='" . ($msgstr["uploadfile"] ?? "Upload File") . "'>";
        echo "<i class='fas fa-cloud-upload-alt'></i> " . ($msgstr["uploadfile"] ?? "Upload File") . "</button>";
        echo "<button type='button' class='abcd-btn-explore' onclick=\"SelectArchivo('tag$Etq','forma1')\" title='" . ($msgstr["selfile"] ?? "Explore Files") . "'>";
        echo "<i class='far fa-folder-open'></i> " . ($msgstr["explore"] ?? "Explore") . "</button>";
        echo "</div>";
    ...
    public static function injectAssets()
    {
        ...
        $str_upload = $msgstr["uploadfile"] ?? "Upload File";
        $str_click = $msgstr["click_browse"] ?? "Click to browse";
        $str_or_drag = $msgstr["or_drag"] ?? "or drag";
        $str_files_here = $msgstr["files_here"] ?? "file(s) here";
        $str_sending = $msgstr["sending"] ?? "Sending...";
        $str_error_upload = $msgstr["error_upload"] ?? "Upload Error";
...
?>
        ...
        <style>
            .abcd-upload-wrapper.dragover {
                background-color: #e8f4f8 !important;
                border-color: #005aa9 !important;
            }

            .abcd-upload-input:focus {
                outline: none;
            ...
            .abcd-modal-dropzone:hover,
            .abcd-modal-dropzone.dragover {
                border-color: #005aa9;
                background: #e8f4f8;
            }

            .abcd-modal-dropzone i {
                font-size: 40px;
                color: #adb5bd;
                margin-bottom: 10px;
            }

            .abcd-modal-dropzone p {
                margin: 0;
                color: #6c757d;
                font-size: 14px;
            }
                ...
                <div id='abcdUploadModal' class='abcd-modal-overlay'>
                    <div class='abcd-modal-content'>
                        <div class='abcd-modal-header'>
                            <h3><?php echo $str_upload; ?>` + (isMultiple ? ' (s)' : '') + `</h3>
                        ...
                        <div class='abcd-modal-body'>
                            ...
                            <div class='abcd-modal-dropzone' id='abcdDropzone_` + tagId + `' onclick='document.getElementById("abcdFileInput_" + "` + tagId + `").click()'>
                                ...
                                <p><strong><?php echo $str_click; ?></strong> <?php echo $str_or_drag; ?> ` + (isMultiple ? '<?php echo $str_files_here; ?>' : 'file(s) here') + `</p>
                            </div>
                            <div id='abcdModalProgress_` + tagId + `' style='display:none; margin-top:15px;'>
                                <div style='font-size:12px; margin-bottom:5px; color:#555;' id='abcdModalStatus_` + tagId + `'><?php echo $str_sending; ?></div>
            ...
            function ABCD_uploadFileAjax(filesArray, tagId, wrapperElement = null, isFromModal = false) {
                ...
                xhr.upload.onprogress = function(e) {
                    if (e.lengthComputable) {
                        ...
                        if (statusText) statusText.innerText = '<?php echo $str_sending; ?> ' + Math.round(percentComplete) + '%';
                    }
                };
                    ...
                    if (xhr.status == 200) {
                        try {
                            ...
                            if (response.success) {
                                ...
                                alert('<?php echo $str_error_upload; ?>: ' + (response.message || 'Failed'));
                                if (isFromModal) {
                            ...
                            // Safely extracts the raw error message returned by PHP (removing the HTML tags)
                            let rawResponse = xhr.responseText ? xhr.responseText.replace(/(<([^>]+)>)/gi, "").trim().substring(0, 250) : "No detailed response.";

                            // Displays the clear translation message + the actual cause returned by the server
                            alert('<?php echo $str_error_parse; ?>\n\n<?php echo $str_details; ?>\n' + rawResponse);

                            // Resets the modal to its initial state so the user can try again
                            if (isFromModal) {
                                document.getElementById('abcdDropzone_' + tagId).style.display = 'block';
                                document.getElementById('abcdModalProgress_' + tagId).style.display = 'none';
                            } else if (wrapperElement) {
                        ...
                        alert('<?php echo $str_error_server; ?>');
                    }
                };
                xhr.send(formData);
            }
