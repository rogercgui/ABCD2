...
$userid = $_SESSION["user_id"] ?? ''; // Evita warning em includes
...
function LeerRegistro() {
    ...
    $myllave = "";
    global $userid, $llamada, $valortag, $maxmfn, $arrHttp, $OS, $Bases, $xWxis, $Wxis, $Mfn, $db_path, $wxisUrl, $empwebservicequerylocation, $empwebserviceusersdb, $db, $EmpWeb, $MD5, $converter_path, $vectorAbrev;

    if (isset($userid)) {
        $checkuser = $userid;

        $mx = $converter_path . " " . $db_path . "users/data/users \"pft=if v20='" . $checkuser . "' then v20,'|',v30,'|',v10,'|',v10^a,'|',v10^b,'|',v18,'|',v620,fi\" now";
        ...
        $mx = $converter_path . " " . $db_path . "users/data/users \"pft=if v20='" . $checkuser . "' then if v610='" . $checkpass . "' then v20,'|',v30,'|',v10,'|',v10^a,'|',v10^b,'|',v18,'|',v620 fi,fi\" now";
    } 
    ...
    if (mb_detect_encoding($textoutmx, 'UTF-8', true) === false) {
        $textoutmx = mb_convert_encoding($textoutmx, "UTF-8", "ISO-8859-1");
    }
...
function getInfoBiblio($CN_item, $DB_item) {
    global $converter_path, $lang, $db_path, $vectorAbrev;
    $formato_obj = $db_path . $DB_item . "/loans/" . $lang . "/loans_display.pft";

    $mx_tit = $converter_path . " " . $db_path . $DB_item . "/data/" . $DB_item . " \"CN_" . $CN_item . "\" \"pft=@" . $formato_obj . "\"  now";
    exec($mx_tit, $outmx_tit);

    // --- INÍCIO DO TRATAMENTO INTELIGENTE DE ENCODING ---

    // Primeiro, verifica se a linha [3] existe e não é nula
    if (isset($outmx_tit[3])) {
        $output_string = $outmx_tit[3];

        // Verifica se a string JÁ é UTF-8 válida
        if (mb_check_encoding($output_string, 'UTF-8')) {
            // Se for, apenas imprime
            echo $output_string;
        } else {
            // Se NÃO for, assume que é ISO-8859-1 e converte para UTF-8
            echo mb_convert_encoding($output_string, 'UTF-8', 'ISO-8859-1');
        }
    }
...
function getRecordStatus() {
    global $empwebservicequerylocation, $empwebserviceobjectsdb, $userid, $EmpWeb, $converter_path, $db_path, $lang, $vectorAbrev;
